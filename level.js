...
const MAX_STEP = 0.05;
const gameAudio = new Audio( 'sounds/sounds_coin.wav')

// plan = nivel
function Level( plan ){

    if( !validateLevel( plan ) ) throw Error( 'The level need to has a player and a coin.')// valida que haya un player y aunque sea una moneda
    this.width = plan[0].length;// longitud de cada linea del nivel
    this.height = plan.length;// longitud del array del nivel
    this.status = null;
    ...
    for( let y = 0; y < this.height; y++ ){
        ...
        for(let x = 0; x < this.width; x++){
            ...
            let Actor = ACTORS[ character ];// valida el actor y lo instancia desde el objeto actors
            if( Actor ) this.actors.push( new Actor( new Vector( x, y), character ));
            if( character === 'x' ) characterType = 'wall';
            if( character === '!' ) characterType = 'lava';

            gridLine.push( characterType ); // convierte cada caracter de la linea en un actor
        }
        this.grid.push( gridLine ); // convierte todo el nivel en actores
    }
