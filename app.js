...
function trackKeys( keyCodes ){
    ...
    function handler( e ){
        if( keyCodes.hasOwnProperty(e.keyCode)){
            let downPressed = e.type === 'keydown';
            pressedKeys[keyCodes[e.keyCode]] = downPressed;
            e.preventDefault;
        }
    }
...
function runAnimation (frameFunction) {
    ...
    function frame (time) {
        ...
        lastTime = time;
        if (!stop) requestAnimationFrame(frame); // anima la ventana( investigar mas sobre esto )
    }
